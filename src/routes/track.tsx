  ...
  return (
    <AppShell>
      <div className="max-w-7xl mx-auto grid lg:grid-cols-[1fr_380px] gap-6">
        {/* Map */}
        <div className="glass-card overflow-hidden relative h-[70vh] min-h-[500px]">
          <StylizedMap progress={progress} />
          <div className="absolute top-4 left-4 glass-panel rounded-2xl px-4 py-2.5 flex items-center gap-2 text-sm font-medium">
            <span className="w-2 h-2 rounded-full bg-success animate-pulse" />
            Live · Updated just now
          </div>
          <div className="absolute bottom-4 left-4 right-4 glass-panel rounded-3xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">ETA</div>
                <div className="font-display text-2xl font-semibold">2h 14m</div>
              </div>
              <div className="text-right">
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Distance left</div>
                <div className="font-display text-2xl font-semibold">142 km</div>
              </div>
              <button className="hidden sm:flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-gradient-to-br from-secondary to-primary text-white text-sm font-medium">
        ...
        <div className="space-y-4">
          <div className="glass-card p-5">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-2">Active Shipment</div>
            <div className="font-display text-xl font-semibold mb-3">RS-48201</div>
                ...
                return (
                  <div key={label} className="flex items-center gap-3">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition-all ${
                      ...
                      {done ? <CheckCircle2 className="w-4 h-4" /> : <span className={`w-1.5 h-1.5 rounded-full ${active ? "bg-secondary animate-pulse" : "bg-muted-foreground/40"}`} />}
                    </div>
          ...
          <Link to="/complete" className="block glass-soft rounded-3xl p-4 text-center text-sm font-medium hover:bg-white/70 transition">
